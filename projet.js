...
function dessinerperformance_moisPSG(){
    ...
    const ctx = canvas.getContext("2d");
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    const performance = [70, 62, 75, 78, 60, 65, 50, 72, 68, 74, 70, 76];
    const mois = ["Jan", "Fév", "Mar", "Avr", "Mai", "Jun", "Jul", "Aoû", "Sep", "Oct", "Nov", "Dec"];
    ctx.strokeStyle = "red";
    ctx.fillStyle = "red";
    ctx.lineWidth = 2;
    ...
    for(let i = 0; i < 12; i++){
        ...
        ctx.font = "12px Arial";
        ctx.textAlign = "center";
        ctx.fillText(mois[i], x, canvas.height - marge + 20);
        ctx.fillStyle = "red";
        ctx.fillText(performance[i].toString() + "%", x, y - 10);
...
function dessinerresulat_moisPSG(){
    const canvas = document.getElementById('resultat_PSG');
    if (!canvas) return;
    ...
    ctx.font = 'bold 18px Arial';
    ctx.textAlign = 'center';
    ctx.fillText('Statistiques des Matchs En Mois de Janvier', 200, 30);
    ctx.strokeStyle = '#333';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(50, 320);
...
function dessinerstat_moisPSG(){
    const canvas = document.getElementById('stat_PSG');
    if (!canvas) return;
    
    const ctx = canvas.getContext('2d');
    const donnees = [15, 12, 5, 7, 3, 4];
    const couleurs = ['#355070', '#6d597a', '#b56576','#e56b6f','#eaac8b','#f4f1de']; 
    const labels = ['Buts', 'Tirs','Passes','Fautes','Corners','Cartons'];
    ctx.clearRect(0, 0, 400, 400);
...
function dessinerperformance_anneePSG(){
    const canvas = document.getElementById("performance_PSG");
    if (!canvas) return;
    
    const ctx = canvas.getContext("2d");
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    const performance = [75, 72, 78, 82, 80];
    const annee = [2020, 2021, 2022, 2023, 2024];
    const marge = 50;
    const largeurUtile = canvas.width - 2 * marge;
    const hauteurUtile = canvas.height - 2 * marge;
...
function dessinerresulat_anneePSG(){
    const canvas = document.getElementById('resultat_PSG');
    if (!canvas) return;
    
    const ctx = canvas.getContext('2d');
    const donnees = [13, 4, 3];
    const couleurs = ['#415a77', '#778da9', '#e0e1dd']; 
    const labels = ['Victoires', 'Nuls', 'Défaites'];
    ctx.clearRect(0, 0, 400, 400);
    for (let i = 0; i < 3; i++) {
...
function dessinerstat_anneePSG(){
    const canvas = document.getElementById('stat_PSG');
    if (!canvas) return;
    
    const ctx = canvas.getContext('2d');
    const donnees = [146, 110, 38, 51, 22, 38];
    const couleurs = ['#355070', '#6d597a', '#b56576','#e56b6f','#eaac8b','#f4f1de']; 
    const labels = ['Buts', 'Tirs','Passes','Fautes','Corners','Cartons'];
    ctx.clearRect(0, 0, 400, 400);
...
function dessinerperformance_moisbayern(){
    ...
    const ctx = canvas.getContext("2d");
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    const performance = [68, 65, 78, 82, 58, 62, 45, 70, 66, 73, 69, 74];
    const mois = ["Jan", "Fév", "Mar", "Avr", "Mai", "Jun", "Jul", "Aoû", "Sep", "Oct", "Nov", "Dec"];
    ctx.strokeStyle = "red";
    ctx.fillStyle = "red";
    ctx.lineWidth = 2;
...
function dessinerresulat_moisbayern(){
    const canvas = document.getElementById('resultat_bayern');
    if (!canvas) return;
...
function dessinerstat_moisbayern(){
    const canvas = document.getElementById('stat_bayern');
    if (!canvas) return;
    
    const ctx = canvas.getContext('2d');
    const donnees = [14, 16, 4, 6, 2, 5];
    const couleurs = ['#355070', '#6d597a', '#b56576','#e56b6f','#eaac8b','#f4f1de']; 
    const labels = ['Buts', 'Tirs','Passes','Fautes','Corners','Cartons'];
    ctx.clearRect(0, 0, 400, 400);
...
function dessinerperformance_anneebayern(){
    const canvas = document.getElementById("performance_bayern");
    if (!canvas) return;
    
    const ctx = canvas.getContext("2d");
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    const performance = [74, 70, 76, 81, 79];
    const annee = [2020, 2021, 2022, 2023, 2024];
    const marge = 50;
    const largeurUtile = canvas.width - 2 * marge;
    const hauteurUtile = canvas.height - 2 * marge;
...
function dessinerresulat_anneebayern(){
    const canvas = document.getElementById('resultat_bayern');
    if (!canvas) return;
    
    const ctx = canvas.getContext('2d');
    const donnees = [12, 6, 4];
    const couleurs = ['#415a77', '#778da9', '#e0e1dd']; 
    const labels = ['Victoires', 'Nuls', 'Défaites'];
    ctx.clearRect(0, 0, 400, 400);
    for (let i = 0; i < 3; i++) {
...
function dessinerstat_anneebayern(){
    const canvas = document.getElementById('stat_bayern');
    if (!canvas) return;
    
    const ctx = canvas.getContext('2d');
    const donnees = [135, 102, 32, 48, 20, 35];
    const couleurs = ['#355070', '#6d597a', '#b56576','#e56b6f','#eaac8b','#f4f1de']; 
    const labels = ['Buts', 'Tirs','Passes','Fautes','Corners','Cartons'];
    ctx.clearRect(0, 0, 400, 400);
